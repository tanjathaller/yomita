...
- **Quelle:** GitHub Action erzeugt weiterhin dieselbe JSON-Struktur (`syncedAt` + `courses[]`, gleiche `Course`-Struktur wie oben, i. d. R. nur `type: "internal"`) und **publiziert** sie in ein **separates Daten-Repo** (oder später einen anderen Endpunkt) — **kein** Commit mehr ins Site-Repo, damit Netlify nicht bei jedem Sync neu baut. Kursdaten kommen aus **Supabase REST**; **Restplatz-Status** wird per **Playwright** aus der sichtbaren YogaFlow-Web-App gelesen (Secret `YOGAFLOW_APP_URL`), damit keine Leserechte auf alle `registrations` nötig sind. Ohne `YOGAFLOW_APP_URL` nutzt das Skript nur den unvollständigen `registrations`-Fallback.
- **Merge:** `getSiteContent()` lädt optional `yogaflowCourses` **pro Request** von `NEXT_PUBLIC_YOGAFLOW_DATA_URL` (`fetch`, `cache: no-store`, Zod-Validierung). Ohne diese URL: Fallback auf lokale `data/yogaflow-courses.json` (Entwicklung). Das Feld **`courses`** bleibt die **manuell** gepflegten Kurse (nicht in der YogaFlow-App). Abschalten: `YOGAFLOW_USE_SYNCED_COURSES=false`.
...
| `faviconUrl`       | string?  | optional; nach Upload oft relative URL `/api/blob-image?key=…` oder externe `https`-URL. Auslieferung: `/favicon.ico` → Rewrite zu `/api/favicon` (`proxy.ts`). Metadaten: `rel="icon"` → `/favicon.ico`. Admin-Vorschau nutzt dieselbe URL-Auflösung wie die öffentliche Seite. |
