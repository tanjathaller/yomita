...
2. **Kursliste (YogaFlow + manuell):** `getSiteContent()` hängt optional `yogaflowCourses` per **Runtime-Fetch** von `NEXT_PUBLIC_YOGAFLOW_DATA_URL` an (Zod wie Datei-Artefakt); ohne URL Fallback auf lokale `data/yogaflow-courses.json`. **`courses`** im KV/JSON sind manuell gepflegte Kurse (ohne App, z. B. Zons, Düsseldorf). **`settings.yogaflowCourseSeries`** beschreibt die App-Serien-Karten (fester Kartentext); Sync-Termine werden nach `matchTitles` gruppiert und in einer **aufklappbaren Liste** pro Karte angezeigt. Danach folgen die manuellen `courses` im gleichen Raster. Fehlt die Serien-Liste leer, setzt `withSortedLists` Defaults. Admin lädt `readSiteContent()` + `withSortedLists` (ohne YogaFlow-Merge), damit der Entwurf keine Sync-Termine persistiert. Deaktivieren: `YOGAFLOW_USE_SYNCED_COURSES=false`. Sync/Publish: Workflow `sync-yogaflow-courses.yml` schreibt ins **Daten-Repo** (Secrets `YOGAFLOW_DATA_REPO`, `YOGAFLOW_DATA_REPO_TOKEN`) — **kein** Push ins Site-Repo, damit Netlify nicht bei jedem Sync deployt; Kursstammdaten Supabase REST, **Restplatz-Status** per **Playwright** (`YOGAFLOW_APP_URL` Secret), damit keine vollständige `registrations`-Lesbarkeit in Supabase nötig ist.
