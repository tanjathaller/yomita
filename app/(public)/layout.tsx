...
export async function generateMetadata(): Promise<Metadata> {
  ...
  return {
    ...
    /** Relativ: wird mit `metadataBase` absolut; entspricht `/favicon.ico`-Rewrite → `/api/favicon`. */
    icons: faviconRaw ? { icon: [{ url: "/favicon.ico" }] } : undefined,
  };
}
...
export default async function PublicLayout({
  ...
  return (
    <>
      ...
      <main
        id="main-content"
        className="relative z-0 isolate min-w-0 w-full max-w-full flex-1 bg-[var(--surface-muted-band)] pt-[var(--site-header-clearance-mobile)] lg:pt-[var(--site-header-clearance)]"
      >
        {children}
      </main>
